...
func register(registry *runtime.Registry) error {
	steps := []struct {
		...
		{`^the forge space (.+?) holds the activity room (.+)$`, spaceHoldsApprovalsRoom},
		{`^the operator is invited to the activity room (.+)$`, invitedToApprovalsRoom},
		{`^the activity room (.+) is encrypted$`, approvalsRoomEncrypted},
		{`^the forge space (.+?) holds the clarifications room (.+)$`, spaceHoldsApprovalsRoom},
		{`^the operator is invited to the clarifications room (.+)$`, invitedToApprovalsRoom},
