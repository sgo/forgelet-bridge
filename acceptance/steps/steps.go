...
func register(registry *runtime.Registry) error {
	steps := []struct {
		...
		{`^the watch's agent runs the watch for the forge roots? (.+)$`, watchAgentRunsFor},
		...
		{`^the installer's output says it installed the route gate, the idler check and the stall watch$`, installerInstalledTheKit},
		...
		{`^the forge root (\S+) carries the route gate, the idler check and the stall watch$`, forgeCarriesTheKit},
		...
		{`^the self-check of the idler check reports the forge as not running$`, idlerSelfCheckReportsTheForgeNotRunning},
